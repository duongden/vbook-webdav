...
export class UserStorage {
  ...
  private async handle(request: Request): Promise<Response> {
    ...
    if (action === '/extension-link') {
      ...
      const token = await this.state.storage.get<string>('extension-token') || supplied;
      await this.state.storage.put('extension-token', token);
      return new Response(token);
    }
    ...
    if (action === '/extension-auth') {
      const token = await this.state.storage.get<string>('extension-token');
      return new Response(null, { status: token && constantTimeEqual(token, request.headers.get('X-Extension-Token') || '') ? 204 : 403 });
    }
