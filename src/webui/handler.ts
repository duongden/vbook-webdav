...
<body>
  ...
  <main class="shell" id="drive" data-usage="${usageBytes}" data-quota="${quotaBytes}">
    ...
    <section class="files-panel" aria-labelledby="files-title">
      ...
      <details class="folder-panel"><summary>Thư mục · chọn để tải tệp vào</summary><div id="folder-list" class="folder-list">${folders.map(folder => html`<button type="button" class="btn folder-target" data-folder="${folder}" title="Tải tệp vào ${folder}">${folder}/</button>`)}</div><p id="folder-empty" class="dialog-note" ${folders.length ? 'hidden' : ''}>Chưa có thư mục. Chọn nút tạo thư mục ở trên.</p></details>
  ...
  <dialog id="folder-dialog" aria-labelledby="folder-title"><form id="folder-form"><h2 id="folder-title">Tạo thư mục</h2><p>Nhập đường dẫn từ thư mục gốc của bạn. Ví dụ: vbookext hoặc vbookext/ten-extension/src.</p><label class="field">Đường dẫn thư mục<input id="folder-path" required maxlength="512" placeholder="vbookext" autocomplete="off"></label><p id="folder-status" class="dialog-note" role="status"></p><div class="dialog-actions"><button type="button" class="btn" id="close-folder">Đóng</button><button type="submit" class="btn btn-primary">Tạo</button></div></form></dialog>
