...
<body>
  ...
  <dialog id="drive-dialog" class="wide-dialog" aria-labelledby="drive-title"><h2 id="drive-title">Google Drive qua WebDAV</h2><p>Mỗi tài khoản có thể liên kết tối đa 20 thư mục Google Drive công khai. Mỗi thư mục dùng API key riêng và có URL WebDAV riêng.</p><div id="drive-connections" class="drive-connections" aria-live="polite"></div><form id="drive-form"><h3>Thêm thư mục Drive</h3><label class="field">Link thư mục Google Drive<input id="drive-folder-url" name="url" type="url" required maxlength="2048" placeholder="https://drive.google.com/drive/folders/…"></label><small class="dialog-note">Trên Google Drive, đặt quyền thư mục thành “Bất kỳ ai có đường liên kết” và quyền “Người xem”.</small><label class="field">Google Drive API key cho thư mục này<input id="drive-api-key" type="password" required autocomplete="new-password" maxlength="256" placeholder="Nhập API key riêng"></label><small class="dialog-note">Key được mã hóa, không hiển thị lại và chỉ dùng cho kết nối này. Trang admin không có quyền xem key.</small><details class="dialog-note"><summary>Cách lấy Google Drive API key</summary><ol><li>Mở <a href="https://console.cloud.google.com/" target="_blank" rel="noopener noreferrer">Google Cloud Console</a>, đăng nhập Google, chọn bộ chọn project → New project → đặt tên → Create.</li><li>Chọn project vừa tạo. Vào APIs &amp; Services → Library, tìm Google Drive API và bấm Enable.</li><li>Vào APIs &amp; Services → Credentials → Create credentials → API key. Đặt tên dễ nhớ.</li><li>Trong API restrictions, chọn Restrict key → Google Drive API → Save. Nếu form yêu cầu giới hạn ngay khi tạo, chọn Google Drive API rồi Create.</li><li>Với máy chủ WebDAV này, Application restrictions chọn None; giới hạn Websites dành cho request từ trình duyệt nên không phù hợp.</li><li>Sao chép key vào ô phía trên. Không dán key vào link thư mục hoặc URL WebDAV.</li></ol><p>Tham khảo <a href="https://developers.google.com/workspace/guides/create-credentials#api-key" target="_blank" rel="noopener noreferrer">hướng dẫn Google</a>.</p></details><p id="drive-status" class="dialog-note" role="status">Đang kiểm tra cấu hình…</p><div class="dialog-actions"><button type="button" class="btn" id="close-drive">Đóng</button><button type="submit" class="btn btn-primary">Thêm liên kết</button></div></form></dialog>
  ...
  <div id="toast" class="toast" role="status" aria-live="polite" popover="manual" hidden><span id="toast-icon" aria-hidden="true"></span><p id="toast-message"></p><button type="button" id="dismiss-toast" aria-label="Đóng thông báo"><svg aria-hidden="true" viewBox="0 0 24 24"><path d="m6 6 12 12M6 18 18 6"/></svg></button></div>
