...
## Cập nhật 2026-09-05: Counter quota và xóa bền vững

Review commit upstream `79087b68e5e405351e75c6a03aeeb45bf7baabde` xác nhận đọc/sửa/ghi counter trong KV làm mất cập nhật khi upload đồng thời. KV cũng chỉ cho ghi cùng key một lần mỗi giây. Quota hiện chuyển sang SQLite-backed Durable Object theo user; KV tiếp tục giữ tài khoản. Đây là thay đổi thay thế quy tắc KV quota cũ trong AGENTS.md.

DO dùng hàng đợi để không interleave các thao tác R2, counter bền vững và cờ dirty ghi trước mutation. Nếu R2 thành công nhưng ghi counter thất bại, lần sau đối soát lại R2. Không coi KV cache là nguồn dung lượng chính xác.

DELETE lưu job trước khi xóa, xử lý tối đa 20 trang/lượt bằng R2 batch delete; còn việc thì trả 503 Retry-After và tiếp tục bằng alarm. Khi gặp lỗi giữa chừng, job vẫn còn để retry. Không dựa vào waitUntil để cam kết xóa hàng chục nghìn file.

Giới hạn upload nội bộ là 100.000.000 byte. Ngoài giới hạn Edge, vẫn kiểm tra Content-Length và độ dài thực tế để bảo vệ giới hạn file/quota thấp hơn do admin đặt. PUT không có Content-Length trả 411; không thay đổi client hay bổ sung multipart upload trong lần sửa này.
